...
base_landing_dir = os.path.abspath(os.path.join(script_dir, '../data/landing/'))
auhouseprices_landing_dir = os.path.abspath(os.path.join(script_dir, "../data/landing/auhouseprices/"))
base_raw_dir = os.path.abspath(os.path.join(script_dir, '../data/raw/'))
auhouseprices_raw_dir = os.path.abspath(os.path.join(script_dir, "../data/raw/auhouseprices/"))
tmp_dir = os.path.abspath(os.path.join(script_dir, '../data/landing/tmp/'))

sitemap_url = "https://www.auhouseprices.com/sitemap.xml"
base_url = "https://www.auhouseprices.com/suburb/market/state/VIC/"
rent_list_url = "https://www.auhouseprices.com/rent/list/VIC/"
...
def get_proxy_url():
    ...
    '''
    username = 'brd-customer-hl_e1a27e6c-zone-datacenter_proxy1'
    password = '3op8m6m48zmb'
    port = 22225
...
async def scrape_listings(partition: list, progress_bar_dict: dict, shared_data: DictProxy):
    '''
    ...
    for url in partition:        
        ...
        async with lock:                
            ...
            input_file = os.path.join(auhouseprices_landing_dir, f"{unique_id}.html")
        ...
        async with lock:
            if html_obj:
                ...
                output_file = os.path.join(auhouseprices_landing_dir, f"{unique_id}.html")            
...
async def get_all_listing_urls(shared_data: DictProxy):
    ...
    with Progress(
        ...
        for url in filtered_urls:
            ...
            # Save the .gz file
            output_gz_file = os.path.join(tmp_dir, gz_filename)
            with open(output_gz_file, 'wb') as f:
                f.write(gz_content)
        
            # Extract the .gz file
            with gzip.open(output_gz_file, 'rb') as f_in:
                input_xml_file = os.path.join(tmp_dir, xml_filename)
    ...
    output_all_urls_file = os.path.join(auhouseprices_landing_dir, 'all_urls.txt')
...
async def main(shared_data: DictProxy):    
    
    rental_listing_urls = await get_all_listing_urls(shared_data)
    ...
    with progress:
        ...
        for partition in partitions:
            task = asyncio.create_task(
                scrape_listings(partition, progress_bar_dict, shared_data)
            )
            tasks.append(task)
...
if __name__ == "__main__":
    ...
    check_make_dir(base_landing_dir)
    check_make_dir(base_raw_dir)
    check_make_dir(auhouseprices_raw_dir)
